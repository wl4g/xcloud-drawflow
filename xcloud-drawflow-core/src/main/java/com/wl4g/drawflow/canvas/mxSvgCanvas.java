...
public class mxSvgCanvas extends mxBasicCanvas {
	...
	/**
	 * Specifies if images should be embedded as base64 encoded strings. Default
	 * is false.
	 */
	 ...
	 * 
	 */
	protected Element createImageElement(double x, double y, double w, double h, String src, boolean aspect, boolean flipH,
			boolean flipV, boolean embedded) {
		Element elem = null;
	...
	public Element drawShape(int x, int y, int w, int h, Map<String, Object> style) {
		...
		if (shape.equals(mxConstants.SHAPE_IMAGE)) {
			...
			String d = "M " + (x + w / 2) + " " + y + " L " + (x + w) + " " + (y + h / 2) + " L " + (x + w / 2) + " " + (y + h)
					+ " L " + x + " " + (y + h / 2);
			...
			d = "M " + x + " " + (y + dy) + " C " + x + " " + (y + 2 * dy) + " " + (x + w) + " " + (y + 2 * dy) + " " + (x + w)
					+ " " + (y + dy);
			...
			if (shape.equals(mxConstants.SHAPE_LABEL)) {
				...
				if (mxUtils.isTrue(style, mxConstants.STYLE_GLASS, false)) {
					...
					// elements in page.
					// Probably its possible that this points to an element in
					// another SVG
