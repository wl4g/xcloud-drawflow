...
import com.wl4g.drawflow.util.mxLightweightLabel;
...
public class mxGraphicsCanvas2D implements mxICanvas2D {
	 ...
	 * changed the match the ratio between px units for rendering HTML and the
	 * units used for rendering other graphics elements. This value is 0.6 on
	 * Linux and 0.75 on all other platforms.
	 */
	...
	public void rect(double x, double y, double w, double h) {
		...
		currentPath.append(new Rectangle2D.Double((state.dx + x) * state.scale, (state.dy + y) * state.scale, w * state.scale,
				h * state.scale), false);
	}
	 ...
	 */
	protected String createHtmlDocument(String text, String align, String valign, int w, int h, boolean wrap, String overflow,
			boolean clip) {
	...
	protected void htmlText(double x, double y, double w, double h, String str, String align, String valign, boolean wrap,
		...
		if (textRenderer != null && rendererPane != null) {
			...
			if (((clip || wrap) && prefWidth > w / HTML_SCALE && w > 0) || (clip && prefHeight > h / HTML_SCALE && h > 0)) {
				// TextWidthDelta is workaround for inconsistent word wrapping
				// in Java
