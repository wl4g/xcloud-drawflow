...
public class GoogleAuthServlet extends AbstractAuthServlet {
	...
	protected Config getConfig() {
		if (CONFIG == null) {
			...
			try {
				clientSerets = Utils.readInputStream(getServletContext().getResourceAsStream(CLIENT_SECRET_FILE_PATH))
						.replaceAll("\n", "");
			} catch (IOException e) {
			...
			try {
				clientIds = Utils.readInputStream(getServletContext().getResourceAsStream(CLIENT_ID_FILE_PATH)).replaceAll("\n",
						"");
			} catch (IOException e) {
	...
	protected String getRefreshTokenCookie(String refreshToken, String tokenCookieVal, String accessToken) {
		...
		return tokenCookieVal; // If we couldn't get the userId, we just return
								// existing tokens such that we don't corrupt
								// them
	}
	...
	protected String processAuthResponse(String authRes, boolean jsonResponse) {
		...
		// In Office Add-in, we don't have access to opened window to attach a
		// function to it,
		// also with the redirect (since we had to open google auth in the same
		// window) we lost Office Messaging.
		// This is due to using Google own file picker instead of creating our
		// own picker
		// (as we did with OneDrive since its picker only support popup windows
		// which is not supported in Office)
		// This is why we load driveLoader.js which define onGDriveCallback and
		// redirects automatically to the page including the picker
		// For other scenarios, we use another function name
		// (onGoogleDriveCallback)
		if (!jsonResponse) {
			...
			res.append("(function() { var authInfo = "); // The following is a
															// json containing
															// access_token
		}
