...
public class MSGraphAuthServlet extends AbstractAuthServlet {
	...
	protected String processAuthResponse(String authRes, boolean jsonResponse) {
		...
		if (!jsonResponse) {
			...
			res.append("		localStorage.setItem('.oneDriveAuthInfo', '{}');"); // setting
																					// this
																					// storage
																					// item
																					// means
			...
			res.append("}");
			res.append(
					"})();</script></head><body><div>Automatic login interrupted. Please close and select OneDrive again.</div></body></html>");
		}
