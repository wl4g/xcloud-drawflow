...
@WebServlet(urlPatterns = { "/github2" })
public class GitHubAuthServlet extends AbstractAuthServlet {
		...
		if (CONFIG == null) {
			...
			try {
				CONFIG.AUTH_SERVICE_URL = Utils
						.readInputStream(getServletContext().getResourceAsStream(AUTH_SERVICE_URL_FILE_PATH))
						.replaceAll("\n", "");
			} catch (IOException e) {
