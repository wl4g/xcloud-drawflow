...
import com.wl4g.drawflow.cache.DrawflowCache;

@SuppressWarnings("serial")
abstract public class AbstractAuthServlet extends HttpServlet {
	...
	protected static DrawflowCache tokenCache;
	...
	protected void addCookie(String name, String val, int age, HttpServletResponse response) {
		response.addHeader("Set-Cookie",
				name + "=" + val + "; Max-Age=" + age + ";path=" + cookiePath + "; Secure; HttpOnly; SameSite=none");
	}

	protected void deleteCookie(String name, HttpServletResponse response) {
		response.addHeader("Set-Cookie",
				name + "= ;path=" + cookiePath + "; expires=Thu, 01 Jan 1970 00:00:00 UTC; Secure; HttpOnly; SameSite=none");
	}
	...
	protected void logout(String tokenCookieName, String tokenCookieVal, HttpServletRequest request,
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("1".equals(stateOnly)) {
			...
			// Chrome blocks this cookie when draw.io is running in an iframe.
			// The cookie is added to parent frame. TODO FIXME
		...
		String secret = null, client = null, redirectUri = null, domain = null, stateToken = null, cookieToken = null,
				version = null, successRedirect = null;
		...
		try {
			...
			try {
				...
				if (cacheKey != null) {
					cookieToken = (String) getTokenCache().get(cacheKey, String.class);
					log.log(Level.INFO, "AUTH-SERVLET: [" + request.getRemoteAddr() + "] Found cookie state (" + cacheKey + " -> "
							+ cookieToken + ")");
					// Delete cookie & cache after being used since it is a
					// single use
			...
			String tokenCookie = TOKEN_COOKIE + client; // Such that we support
														// multiple client ids
			...
			// Non GAE runtimes are excluded from state check. TODO Change GAE
			// stub to return null from CacheFactory

			// else if
			// (!"Non".equals(com.google.appengine.api.utils.SystemProperty.environment.get())
			// && (stateToken == null || !stateToken.equals(cookieToken))) {

			else if ((stateToken == null || !stateToken.equals(cookieToken))) {
				log.log(Level.WARNING, "AUTH-SERVLET: [" + request.getRemoteAddr() + "] STATE MISMATCH (state: " + stateToken
						+ " != cookie: " + cookieToken + ")");
				response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
			} else {
				Response authResp = contactOAuthServer(CONFIG.AUTH_SERVICE_URL, code, refreshToken, secret, client, redirectUri,
						successRedirect != null, 1);

				response.setStatus(authResp.status);

				if (authResp.refreshToken != null) {
					addCookie(tokenCookie, getRefreshTokenCookie(authResp.refreshToken, tokenCookieVal, authResp.accessToken),
							TOKEN_COOKIE_AGE, response);
				}
	...
	private Response contactOAuthServer(String authSrvUrl, String code, String refreshToken, String secret, String client,
			String redirectUri, boolean directResp, int retryCount) {
	...
	protected static final DrawflowCache getTokenCache() {
		if (isNull(tokenCache)) {
			synchronized (AbstractAuthServlet.class) {
				if (isNull(tokenCache)) {
					tokenCache = SpringContextHolder.getBean(DrawflowCache.class);
				}
			}
		}
		return tokenCache;
