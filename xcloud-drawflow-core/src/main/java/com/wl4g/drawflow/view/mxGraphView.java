...
import com.wl4g.drawflow.view.mxPerimeter.mxPerimeterFunction;
...
public class mxGraphView extends mxEventSource {
	...
	public void scaleAndTranslate(double scale, double dx, double dy) {
		...
		fireEvent(new mxEventObject(mxEvent.SCALE_AND_TRANSLATE, "scale", scale, "previousScale", previousScale, "translate",
	...
	public double getWordWrapWidth(mxCellState state) {
		...
		if (horizontal) {
			w = (state.getWidth() / scale) - 2 * mxConstants.LABEL_INSET - 2 * mxUtils.getDouble(style, mxConstants.STYLE_SPACING)
