...
public class mxEdgeStyle {
	...
	public interface mxEdgeStyleFunction {
		...
		/**
		 * Implements an edge style function. At the time the function is
		 * called, the result array contains a placeholder (null) for the first
		 * absolute point, that is, the point where the edge and source terminal
		 * are connected. The implementation of the style then adds all
		 * intermediate waypoints except for the last point, that is, the
		 * connection point between the edge and the target terminal. The first
		 * ant the last point in the result array are then replaced with
		 * mxPoints that take into account the terminal's perimeter and next
		 * point on the edge.
		 * 
		 * @param state
	...
	public static mxEdgeStyleFunction EntityRelation = new mxEdgeStyleFunction() {
		 ...
		 * 
		 * @see
		 *com.wl4g.drawflow.view.mxEdgeStyle.mxEdgeStyleFunction#apply(com.mxgraph.
		 * view.mxCellState,com.wl4g.drawflow.view.mxCellState,
		 *com.wl4g.drawflow.view.mxCellState, java.util.List, java.util.List)
		 */
	...
	/**
	 * Uses either SideToSide or TopToBottom depending on the horizontal flag in
	 * the cell style. SideToSide is used if horizontal is true or unspecified.
	 */
	...
	public static mxEdgeStyleFunction SegmentConnector = new mxEdgeStyleFunction() {
		...
		public void apply(mxCellState state, mxCellState source, mxCellState target, List<mxPoint> hints, List<mxPoint> result) {
			...
			if (hints != null && hints.size() > 0) {
				...
				if (horizontal
						&& ((pts.get(0) != null && pts.get(0).getY() != hint.getY()) || (pts.get(0) == null && source != null
			...
			if (horizontal && ((pts.get(lastInx) != null && pts.get(lastInx).getY() != hint.getY()) || (pts.get(lastInx) == null
					&& target != null && (hint.getY() < target.getY() || hint.getY() > target.getY() + target.getHeight())))) {
				result.add(new mxPoint(pt.getX(), hint.getY()));
	...
	/**
	 * Overriden routing patterns for orthogonal connections where the vertices
	 * have
	 */
	...
	public static mxEdgeStyleFunction OrthConnector = new mxEdgeStyleFunction() {
		...
		public void apply(mxCellState state, mxCellState source, mxCellState target, List<mxPoint> points, List<mxPoint> result) {
			...
			if (source != null && target != null) {
				...
				// Work out which faces of the vertices present against each
				// other
				// in a way that would allow a 3-segment connection if port
				// constraints
				// permitted.
				...
				// Work through the preferred orientations by relative
				// positioning
				// of the vertices and list them in preferred and available
				// order
				...
				int lastOrientation = (dir[0] & (mxConstants.DIRECTION_MASK_EAST | mxConstants.DIRECTION_MASK_WEST)) > 0 ? 0 : 1;
				...
				for (int i = 0; i < routePattern.length; i++) {
					...
					if ((sou || tar) && side < 9) {
						...
						// Which center we're travelling to depend on the
						// current direction
