...
import com.wl4g.drawflow.model.mxCellPath;
import com.wl4g.drawflow.model.mxICell;
import com.wl4g.drawflow.util.mxDomUtils;
...
public class mxCodec {
	 ...
	 * {@link #document}. If the object is not known then
	 * {@link #lookup(String)} is used to find an object. If no object is found,
	 * then the element with the respective ID from the document is parsed using
	 * {@link #decode(Node)}.
	 * 
	 * @param id
	 ...
	 * reference first and if that returns null handles the object as an mxCell
	 * by returning their IDs using mxCell.getId. If no ID exists for the given
	 * cell, then an on-the-fly ID is generated using mxCellPath.create.
	 * 
	 * @param obj
	 ...
	 * higher-level function that needs to be explicitely used by the respective
	 * object encoders (eg. mxModelCodec, mxChildChangeCodec and
	 * mxRootChangeCodec). This implementation writes the given cell and its
	 * children as a (flat) sequence into the given node. The children are not
	 * encoded if the optional includeChildren is false. The function is in
	 * charge of adding the result into the given node and has no return value.
	 * 
	 * @param cell
	 ...
	 *            Boolean indicating if the method should include all
	 *            descendents.
	 */
	 ...
	 * structure in the cells. Returns a new <mxCell> instance that represents
	 * the given node.
	 * 
	 * @param node
	 *            XML node that contains the cell data.
	 * @param restoreStructures
	 *            Boolean indicating whether the graph structure should be
	 *            restored by calling insert and insertEdge on the parent and
	...
	/**
	 * Sets the attribute on the specified node to value. This is a helper
	 * method that makes sure the attribute and value arguments are not null.
	 *
	 * @param node
