...
public class mxObjectCodec {
	 ...
	 * classes are encoded and nodes are decoded. For classes to work with this
	 * the codec registry automatically adds an alias for the classname if that
	 * is different than what this returns. The default implementation returns
	 * the classname of the template class.
	 * 
	 ...
	 *            write is true if the field is being encoded, else it is being
	 *            decoded.
	 ...
	 * @param isWrite
	 *            Boolean indicating if the field is being encoded or decoded.
	 *            isWrite is true if the field is being encoded, else it is
	 *            being decoded.
	 ...
	 * <li>If obj is an array and the variable name is numeric (ie. an index)
	 * then it is not encoded.</li>
	 ...
	 * <li>Else, if {@linkcom.wl4g.drawflow.io.mxCodec#isEncodeDefaults()} is true or
	 * the value differs from the template value, then ...
	 * <ul>
	 * <li>... if obj is not an array, then the value is mapped to an
	 * attribute.</li>
	 * <li>... else if obj is an array, the value is mapped to an add child with
	 * a value attribute or a text child node, if the value is a function.</li>
	 * </ul>
	 * </li>
	 * </ul>
	 * 
	 * If no ID exists for a variable in {@link #idrefs} or if an object cannot
	 * be encoded, a warning is logged.
	 * 
	 * @param enc
	 ...
	 * using {@link #encodeFields(mxCodec, Object, Node)} and
	...
	/**
	 * Converts the given value according to the mappings and id-refs in this
	 * codec and uses
	 * {@link #writeAttribute(mxCodec, Object, String, Object, Node)} to write
	 * the attribute into the given node.
	 * 
	 * @param enc
	 *            Codec that controls the encoding process.
	 * @param obj
	...
	protected Method getAccessor(Object obj, Field field, boolean isGetter) {
		...
		if (method == null) {
			// this should be considered an error in the scope of this method,
			// but the
			// calling code already depends on this method failing softly to
			// filter
			// non-serializable properties, so it gets called for static fields
			// (mxCell.serialVersionUID), non-transient-but-probably-should-be
			// fields
			// (mxCell.children, mxCell.edges)
			// the proper fix is to rewrite the whole thing to use Introspector,
			// like
			// encodeFields already intends, so for now let's just log at a
			// lower level
	...
	/**
	 * Sets the value of the field with the specified name in the specified
	 * object instance.
	 */
	 ...
	 * to the following rules: - If the variable name is in <exclude> or if the
	 * attribute name is "id" or "as" then it is ignored. - If the variable name
	 * is in <idrefs> then <mxCodec.getObject> is used to replace the reference
	 * with an object. - The variable name is mapped using a reverse <mapping>.
	 * - If the value has a child node, then the codec is used to create a child
	 * object with the variable name taken from the "as" attribute. - If the
	 * object is an array and the variable name is empty then the value or child
	 * object is appended to the array. - If an add child has no value or the
	 * object is not an array then the child text content is evaluated using
	 * <mxUtils.eval>.
	 * 
	...
	/**
	 * Returns the template instance for the given field. This returns the value
	 * of the field, null if the value is an array or an empty collection if the
	 * value is a collection. The value is then used to populate the field for a
	 * new instance. For strongly typed languages it may be required to override
	 * this to return the correct collection instance based on the encoded
	 * child.
	 */
	...
	/**
	 * Sets the decoded child node as a value of the given object. If the object
	 * is a map, then the value is added with the given fieldname as a key. If
	 * the fieldname is not empty, then setFieldValue is called or else, if the
	 * object is a collection, the value is added to the collection. For
	 * strongly typed languages it may be required to override this with the
	 * correct code to add an entry to an object.
	 */
	 ...
	 * function is used in {@link #decode(mxCodec, Node)} to perform the default
	 * decoding into the given object.
	 * 
	 * @param dec
