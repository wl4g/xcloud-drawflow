...
package com.wl4g.drawflow.layout;
...
import com.wl4g.drawflow.view.mxGraphView;
...
public class mxOrganicLayout extends mxGraphLayout {
	...
	/**
	 * Whether or not the distance between edge and nodes will be calculated as
	 * an energy cost function. This function is CPU intensive and is best only
	 ...
	 * without it are required, it is best to have a few cycles at the start of
	 * the algorithm using it, then use it intermittantly through the rest of
	 * the layout.
	 */
	...
	/**
	 * Whether or not node distribute will contribute an energy cost where nodes
	 * are close together. The function is moderately CPU intensive.
	 */
	...
	/**
	 * The average amount of area allocated per node. If <code> bounds</code> is
	 * not set this value mutiplied by the number of nodes to find the total
	 * graph area. The graph is assumed square.
	 */
	protected double averageNodeArea = 160000;

	/**
	 * The radius below which fine tuning of the layout should start This
	 * involves allowing the distance between nodes and edges to be taken into
	 * account in the total energy calculation. If this is set to zero, the
	 * layout will automatically determine a suitable value
	 */
	...
	/**
	 * Cost factor applied to energy calculations involving the distance nodes
	 * and edges. Increasing this value tends to cause nodes to move away from
	 * edges, at the partial cost of other graph aesthetics.
	 ...
	 * distribution of the graph. Increasing this value tends to result in a
	 * better distribution of nodes across the available space, at the partial
	 * cost of other graph aesthetics. <code>isOptimizeNodeDistribution</code>
	 ...
	 * notional border of the graph. Increasing this value results in nodes
	 * tending towards the centre of the drawing space, at the partial cost of
	 * other graph aesthetics. <code>isOptimizeBorderLine</code> must be true
	 * for border repulsion to be applied.
	 */
	 ...
	 * aesthetics. <code>isOptimizeEdgeLength</code> must be true for edge
	 * length shortening to be applied.
	 */
	...
	/**
	 * Array of the x portion of the normalised test vectors that are tested for
	 * a lower energy around each vertex. The vector of the combined x and y
	 * normals are multipled by the current radius to obtain test points for
	 * each vector in the array.
	 */
	protected double[] xNormTry;

	/**
	 * Array of the y portion of the normalised test vectors that are tested for
	 * a lower energy around each vertex. The vector of the combined x and y
	 * normals are multipled by the current radius to obtain test points for
	 * each vector in the array.
	 */
	...
	/**
	 * Whether or not fine tuning is on. The determines whether or not node to
	 * edge distances are calculated in the total system energy. This cost
	 * function , besides detecting line intersection, is a performance
	 * intensive component of this algorithm and best left to optimization
	 * phase. <code>isFineTuning</code> is switched to <code>true</code> if and
	 * when the <code>fineTuningRadius</code> radius is reached. Switching this
	 * variable to <code>true</code> before the algorithm runs mean the node to
	 * edge cost function is always calculated.
	 */
	...
	public void execute(Object parent) {
		...
		// Remove edges that do not have both source and target terminals
		// visible
		for (int i = 0; i < vertices.length; i++) {
			Object[] edges = mxGraphModel.getEdges(model, vertices[i], false, true, false);

			for (int j = 0; j < edges.length; j++) {
				// Only deal with sources. To be valid in the layout, each edge
				// must be attached
				// at both source and target to a vertex in the layout. Doing
	...
	/**
	 * The main round of the algorithm. Firstly, a permutation of nodes is
	 * created and worked through in that random order. Then, for each node a
	 * number of point of a circle of radius <code>moveRadius</code> are
	 * selected and the total energy of the system calculated if that node were
	 * moved to that new position. If a lower energy position is found this is
	 * accepted and the algorithm moves onto the next node. There may be a
	 * slightly lower energy value yet to be found, but forcing the loop to
	 * check all possible positions adds nearly the current processing time
	 * again, and for little benefit. Another possible strategy would be to take
	 * account of the fact that the energy values around the circle decrease for
	 * half the loop and increase for the other, as a general rule. If part of
	 * the decrease were seen, then when the energy of a node increased, the
	 * previous node position was almost always the lowest energy position. This
	 * adds about two loop iterations to the inner loop and only makes sense
	 * with 16 tries or more.
	 */
	protected void performRound() {
		...
		for (int i = 0; i < v.length; i++) {
			...
			for (int j = 0; j < triesPerCell; j++) {
				...
				double energyDelta = calcEnergyDelta(index, oldNodeDistribution, oldEdgeDistance, oldEdgeCrossing, oldBorderLine,
	 ...
	 * 
	 */
	protected double calcEnergyDelta(int index, double oldNodeDistribution, double oldEdgeDistance, double oldEdgeCrossing,
	...
	/**
	 * This method calculates the energy of the distance of the specified node
	 * to the notional border of the graph. The energy increases up to a limited
	 ...
	 * involves calling <code>getEdgeLength</code> for all edges connected to
	 * the specified node
	 * 
	 * @param node
	...
	/**
	 * Obtains the energy cost function for the specified node being moved. This
	 * involves calling <code>getEdgeCrossing</code> for all edges connected to
	 * the specified node
	 * 
	 * @param node
	 *            the node whose connected edges cost functions are to be
	 ...
	 * crossing any other edges. Each crossing add a constant factor to the
	 * total energy
	 * 
	 * @param i
	 *            the index of the edge in the array <code>e</code>
				...
				if (j != i) {
					...
					if (((iP1X != jP1X) && (iP1Y != jP1Y)) && ((iP1X != jP2X) && (iP1Y != jP2Y))
						...
						boolean intersects = ((Line2D.relativeCCW(iP1X, iP1Y, iP2X, iP2Y, jP1X, jP1Y) != Line2D.relativeCCW(iP1X,
								...
								&& (Line2D.relativeCCW(jP1X, jP1Y, jP2X, jP2Y, iP1X, iP1Y) != Line2D.relativeCCW(jP1X, jP1Y, jP2X,
										jP2Y, iP2X, iP2Y)));
	 ...
	 * Edges. This version of the edge distance cost calculates the energy cost
	 * from a specified <strong>node</strong>. The distance cost to all
	...
	protected double getEdgeDistanceFromNode(int i) {
		...
		if (isOptimizeEdgeDistance && isFineTuning) {
			...
			for (int j = 0; j < edges.length; j++) {
				...
				double distSquare = Line2D.ptSegDistSq(v[e[edges[j]].source].x, v[e[edges[j]].source].y, v[e[edges[j]].target].x,
	...
	/**
	 * Obtains the energy cost function for the specified node being moved. This
	 * involves calling <code>getEdgeDistanceFromEdge</code> for all edges
	 ...
	 * This method calculates the energy of the distance between Cells and
	 * Edges. This version of the edge distance cost calculates the energy cost
	 * from a specified <strong>edge</strong>. The distance cost to all
	...
	public class CellWrapper {
		...
		/**
		 * All edge that repel this cell, only used for nodes. This array is
		 * equivalent to all edges unconnected to this node
		 */
		protected int[] relevantEdges = null;

		/**
		 * the index of all connected edges in the <code>e</code> array to this
		 * node. This is only used for nodes.
		 */
		 ...
		 * approxNodeDimensions is true on the layout this value holds the width
		 * of the node squared
		 */
		protected double radiusSquared;

		/**
		 * The height of the node squared, only used if approxNodeDimensions is
		 * set to true.
		 */
