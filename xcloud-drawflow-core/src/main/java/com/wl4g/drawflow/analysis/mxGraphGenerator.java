...
import com.wl4g.drawflow.costfunction.mxCostFunction;
import com.wl4g.drawflow.costfunction.mxDoubleValCostFunction;
import com.wl4g.drawflow.generatorfunction.mxGeneratorFunction;
import com.wl4g.drawflow.generatorfunction.mxGeneratorRandomFunction;
import com.wl4g.drawflow.model.mxGeometry;
...
public class mxGraphGenerator {
	 ...
	 * @return a bipartite graph with group 1 containing
	 *         <b>numVerticesGroup1</b> vertices and group 2 containing
	 *         <b>numVerticesGroup2</b>
	 */
	 ...
	 * mxGraphCreator.getBipartiteGraph() only after creating the graph
	 * 
	 * @param aGraph
	 * @param numVerticesGroup1
	 *            - number of vertices in group 1
	...
	public void getKnightGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
		...
		for (int i = 0; i < (xDim * yDim); i++) {
			...
			for (int j = 0; j < neighborMoves.length; j++) {
				// connect current with the possible move that has minimum
				// number of its (possible moves)
				...
				// that vertex becomes the current vertex and we repeat until no
				// possible moves
			}
		}
	};
	 ...
	 * @return a list of ALL vertexes which would be valid moves from the
	 *         current position, regardless if they were visited or not Note
	...
	/**
	 * use this only with the grid graph, and various chess-board graphs,
	 * because of vertex ordering
	 * 
	 * @param xDim
	 *            x dimension of chess-board, size starts from 1
	 * @param yDim
	 ...
	 * @return int[x,y] where x and y are the coordinates in the grid or
	 *         chess-board Note that both dimensions must be positive
	 */
	...
	/**
	 * use this only with the grid graph and various chess-board graphs, because
	 * of vertex ordering
	 * 
	 * @param vertices
	 * @param xDim
	 ...
	 * @param yCoord
	 *            y coordinate on the chess-board, coordinate starts from 1
	 * @return vertex on the desired coordinates. Note that both dimensions and
	 ...
	 * @param yDim
	 * @param weights
	 *            Return a king graph of size <b>xDim</b> x <b>yDim</b> Note
	 *            that the minimum size is 4x4
	 */
	...
	/**
	 * Sets the physical spacing between vertices in a path graph. This works
	 * for now only for a graph generated with mxGraphCreator.getPathGraph()
	 * only after creating the graph
	 * 
	 * @param aGraph
	 * @param spacing
	...
	/**
	 * Sets the physical size of a star graph. This works for now only for a
	 * graph generated with mxGraphCreator.getStarGraph() and getWheelGraph()
	 * 
	 * @param aGraph
	 * @param graphSize
	 */
	 ...
	 * @param aGraph
	 * @param numVertices
	 *            Returns a wheel graph. Note that numVertices has to be at
	 *            least 4.
	 */
	 ...
	 *            - number of branches (minimum >= 2)
	 * @param branchSize
	 *            - number of vertices in a single branch (minimum >= 2) Returns
	 *            a windmill graph
	 */
	...
	// NOTE the inside code is delicate, so please change it only if you know
	// what you're doing
	/**
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < numVerticesInBranch; j++) {
				...
				if (isBranchSizeEven && j == middleIndex - 1) {
					...
					if (j + 1 < middleIndex) {
						// before middle
						currRatio = ((double) (i - 1f / Math.pow(currSize, 0.25) + 0.00000000000015f * Math.pow(currSize, 4))
								/ (double) numVertexesInPerimeter);
					} else {
						// after middle
						currRatio = ((double) (i + 1f / Math.pow(currSize, 0.25) - 0.00000000000015f * Math.pow(currSize, 4))
	...
	/**
	 * A helper function that calculates the ring size for a windmill graph,
	 * based on the index of a vertex in a brach and branch size. - for internal
	 * use
	 * 
	 * @param currVertex
	 *            - starting from 1
	 ...
	 *            number of vertexes
	 * @param numEdges
	 *            number of edges (may be inaccurate if <b>forceConnected</b> is
	 *            set to true
	 * @param allowSelfLoops
	 *            if true, there will be a chance that self loops will be
	 *            generated too
	 ...
	 *            if true the resulting graph will be always connected, but this
	 *            may alter <b>numEdges</b>
	 */
	 ...
	 * Used mostly when creating new edges during graph generation.
	 * 
	 * @param aGraph
	 * @return
	 */
	 ...
	 * @param forceSimple
	 *            if true, a non-simple graph is made simple Calculates one
	 *            spanning tree of graph, which doesn't have to be but can be
	 *            minimal (this is faster than minimal spanning tree, so if you
	 *            need any spanning tree, use this one) Self loops and multiple
	 *            edges are automatically removed! Also, unconnected graphs are
	 *            made connected!
	 * @throws StructuralException
	 *             the graph has to be simple (no self-loops and no multiple
	 *             edges)
	 */
	...
	// TODO make a double check to avoid unnecessary cases of throwing an
	// exception (if the algorithm can't work out a solution, try a mirrored
	// strategy)
	/**
	 * @param aGraph
	 * @param xDim
	 ...
	 * @throws StructuralException
	 *             not all size combinations are allowed, see wikipedia for a
	 *             more detailed explanation Returns a Knight's Tour graph
	 */
	public void getKnightTour(mxAnalysisGraph aGraph, int xDim, int yDim, int startVertexValue) throws StructuralException {
		...
		// the main loop
		while (nextMove != null) {
			// connect current with the possible move that has minimum number of
			// its (possible moves)
