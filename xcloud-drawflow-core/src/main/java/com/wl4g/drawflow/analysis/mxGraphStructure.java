...
public class mxGraphStructure {
	...
	public static void makeConnected(mxAnalysisGraph aGraph) {
		// an early check, to avoid running getGraphComponents() needlessly,
		...
		if (isTree(aGraph)) {
			...
			for (int i = 0; i < bFSList.size(); i++) {
				...
				for (int j = 0; j < neighbors.length; j++) {
					...
					if (childIndex > i) {
						// parentVertex is parent of currVertex, so the edge
						// must be directed from parentVertex to currVertex
	 ...
	 * @param vertexOne
	 * @param vertexTwo
	 * @return an edge that directly connects <b>vertexOne</b> and
	 *         <b>vertexTwo</b> regardless of direction, null if they are not
	 *         connected directly
	 */
	 ...
	 *         edges. It could be considered a "leaf" in a directed graph, but
	 *         this definition isn't formal.
	 */
	...
	public static void complementaryGraph(mxAnalysisGraph aGraph) {
		...
		for (int i = 0; i < vertexCount; i++) {
			...
			for (int j = 0; j < vertexCount; j++) {
				...
				boolean shouldConnect = true; // the decision if the two current
												// vertexes should be connected
