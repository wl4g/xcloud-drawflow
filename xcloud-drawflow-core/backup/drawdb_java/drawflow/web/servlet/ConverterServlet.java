...
public class ConverterServlet extends HttpServlet {
	...
	private static final String BOUNDARY = "----WebKitFormBoundary6XTanBMjO0kFwa3p"; // FIXME
																						// The
																						// boundary
																						// should
																						// not
	...
	private void readApiKey() {
		if (API_KEY == null) {
			try {
				API_KEY = Utils.readInputStream(getServletContext().getResourceAsStream(API_KEY_FILE_PATH)).replaceAll("\n", "");
			} catch (IOException e) {
		...
		if (inputformat == null || outputformat == null || fileContent == null) {
			...
			try {
				...
				if (inputformat.equals("emf") && bytesRead >= 40) {
					// Read Frame from EMF header (the rectangular
					// inclusive-inclusive dimensions, in .01 millimeter units,
					// of a rectangle that surrounds the image stored in the
					// metafile.)
				...
				try {
					...
					log.log(Level.CONFIG, "EMF-CONVERT, domain: " + domain + " ,Filename: " + fileName != null ? fileName
							: "" + ", size: " + bytesRead);
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void addParameterHeader(String name, String fileName, DataOutputStream postRequest) throws IOException {
		...
		postRequest.writeBytes("Content-Disposition: form-data; name=\"" + name + "\""
				+ (fileName != null ? "; filename=\"" + fileName + "\"" + CRLF + "Content-Type: application/octet-stream" : "")
				+ CRLF);
		postRequest.writeBytes(CRLF);
	}
}
