...
import com.wl4g.drawflow.io.mxGraphMlCodec;
import com.wl4g.drawflow.io.gliffy.importer.GliffyDiagramConverter;
...
public class OpenServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if (request.getContentLength() < Constants.MAX_REQUEST_SIZE) {
				...
				if (filename.toLowerCase().endsWith(".png")) {
					...
					if (ENABLE_GRAPHML_SUPPORT && upfile.matches(graphMlRegex)) {
						// Creates a graph that contains a model but does not
						// validate
						// since that is not needed for the model and not
						// allowed on GAE
				...
				if (xml == null) {
					xml = (upfile == null) ? request.getParameter("data") : upfile;
				}

				String ref = request.getHeader("referer");

				if (ref != null && ref.toLowerCase().matches("https?://([a-z0-9,-]+[.])*quipelements[.]com/.*")) {
				...
				if (!format.equals("xml")) {
					if (xml == null || xml.length() == 0) {
						...
						// Workaround for replacement char and null byte in IE9
						// request
						xml = xml.replaceAll("[\\uFFFD\\u0000]*", "");
						writeScript(writer, "try{window.parent.setCurrentXml(decodeURIComponent('" + encodeString(xml)
								+ "'), decodeURIComponent('" + encodeString(filename)
			...
			writeScript(writer, "window.parent.showOpenAlert('Out of memory');");
		} catch (Exception e) {
	...
	protected String encodeString(String s) {
		return StringEscapeUtils.escapeEcmaScript(Utils.encodeURIComponent(s, CHARSET_FOR_STRING_URL_ENCODING));
	};

	/**
	 * Writes the given string as a script in a HTML page to the given print
	 * writer.
	 */
