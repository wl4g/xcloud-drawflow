...
@WebServlet(urlPatterns = { "/microsoft" })
public class MSGraphAuthServlet extends AbstractAuthServlet {
		...
		if (CONFIG == null) {
			...
			try {
				CONFIG.DEV_CLIENT_SECRET = Utils
						.readInputStream(getServletContext().getResourceAsStream(DEV_CLIENT_SECRET_FILE_PATH))
						.replaceAll("\n", "");
			} catch (IOException e) {
			...
			try {
				CONFIG.DEV_CLIENT_ID = Utils.readInputStream(getServletContext().getResourceAsStream(DEV_CLIENT_ID_FILE_PATH))
						.replaceAll("\n", "");
			} catch (IOException e) {
	...
	protected String processAuthResponse(String authRes, boolean jsonResponse) {
		...
		if (!jsonResponse) {
			res.append(
					"<!DOCTYPE html><html><head><script src=\"https://appsforoffice.microsoft.com/lib/1.1/hosted/office.js\" type=\"text/javascript\"></script><script>");
