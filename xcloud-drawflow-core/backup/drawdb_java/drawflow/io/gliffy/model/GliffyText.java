...
import com.wl4g.drawflow.io.gliffy.importer.PostDeserializer;
...
public class GliffyText implements PostDeserializer.PostDeserializable {
	...
	public String getStyle(float x, float y) {
		...
		// I hate magic numbers, but -7 seams to fix all text top padding when
		// valign is not middle
		...
		if (hposition.equals("left")) {
			...
			if (halign != null) {
				...
				if (halign.equalsIgnoreCase("right")) {
					// Workaround for word wrapping where no wrapping occurs in
					// Gliffy is to
					// make room for additional chars if we know the alignment
					// ignores x
					x = 0;
				}
			} else {
		...
		// Removes default global spacing (workaround for unwanted line
		// wrapping)
		...
		// We should wrap only if overflow is none. (TODO better support left &
		// right overflow)
			...
			if (style != null) {
				...
				if (!m2.find()) {
					if (html.substring(m.end(), m.end() + 5).equalsIgnoreCase("<span")) {
						...
						// Overrides line-height with default value in child
						// span elements
	...
	/**
	 * Extracts horizontal text alignment from html and removes it so it does
	 * not interfere with alignment set in mxCell style
	 * 
	...
	/**
	 * Replaces all occurrences of style="text-align: {position}" with
	 * align="{position}" This enables per-line horizontal alignment in all
	 * browsers
	 */
