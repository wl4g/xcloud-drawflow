...
import com.wl4g.drawflow.io.gliffy.importer.PostDeserializer.PostDeserializable;
...
public class GliffyObject implements PostDeserializable {
	...
	static {
		...
		// Format [xShift, yShift, widthShift, heightShift, DONT REPOSITION
		// TEXT] values between ]-1, 1[ means percentage
		...
		SHAPES_COORD_FIX.put("com.gliffy.shape.uml.uml_v2.deployment.execution_environment_node",
				new double[] { 0, -10, 10, 10 });
	...
	public String getText() {
		...
		// TODO These values are heuristics based on analyzing many files. 6 is
		// a range from 2 to 6 so used the maximum
		int widthDiff = "none".equals(text.overflow) ? -3 : 6;
		return "<div style='width: " + (width + widthDiff) + "px;height:auto;word-break: break-word;'>" + text.getHtml()
				+ "</div>";
	...
	/**
	 * Some shapes like UML package, class and interface do not have a graphic
	 * object but instead rely on graphic of their children. In that case,
	 * graphic is the same for all children
	 * 
	...
	public boolean isGroup() {
		return (hasChildren() && ((uid != null && (GROUP_SHAPES.contains(uid) || uid.startsWith("com.gliffy.shape.table")))
				// Since we treat text in a different way (added as cell value
				// instead of another child cell, this is probably the best way
				// to detect groups when uid is null)
	...
	public boolean isShape() {
		if (graphic != null) {
			...
			// some UML shapes do not have a graphic,instead their graphic type
			// is determined by their first child
	...
	public String getGradientColor() {
		...
		// Gradient colors are lighter version of the fill color except for
		// radial
		// venn shapes, where white is used with a radial gradient (we use
		// linear)
	...
	/**
	 * Some Gliffy diagrams have groups whose children have negative
	 * coordinates. This is a problem in draw.io as they get set to 0. This
	 * method expands the groups left and up and adjusts children's coordinates
	 * so that they are never less than zero.
	 */
	...
	/**
	 * Fix for https://desk.draw.io/helpdesk/tickets/5205 Since Gliffy can have
	 * groups whose children interleave in terms of z order and we can't, we
	 * assign the group a z order to that of it's highest ordered rectangle
	 * child
	 */
	private void adjustZOrder() {
		...
		for (GliffyObject c : children) {
			if (c.uid != null && c.uid.equals("com.gliffy.shape.basic.basic_v1.default.rectangle") && c.x == 0 && c.y == 0
	...
	public void adjustGeo(mxGeometry geo) {
		double[] arr = SHAPES_COORD_FIX
				.get(uid != null ? uid : (graphic != null && graphic.getShape() != null ? graphic.getShape().tid : null));
	...
	public boolean isUseFillColor4StrokeColor() {
		return FILLCLR_IS_STROKECLR_SHAPES
				.contains(uid != null ? uid : (graphic != null && graphic.getShape() != null ? graphic.getShape().tid : null));
	}

	/**
	 * @return true If gliffyObject is Frame then always stick text on top left
	 *         corner.
	 */
